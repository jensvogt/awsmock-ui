...
@Component({
    selector: 'application-cpu-chart-component',
    templateUrl: './application-cpu-chart.component.html',
    standalone: true,
    imports: [
        ChartComponent,
        MatCard,
    ...
    styleUrls: ['./application-cpu-chart.component.scss']
})
export class ApplicationCpuChartComponent implements OnInit {

    public applicationCpuChartOptions!: Partial<ChartOptions> | any;
    ...
    ngOnInit(): void {
        ...
        this.loadApplicationCpuChart();
    }

    loadApplicationCpuChart() {

        let start = this.chartService.getStartTime(this.selectedTimeRange);
        let end = this.chartService.getEndTime();
        this.monitoringService.getMultiCounters('application_cpu_usage', 'application', start, end, 5)
            .subscribe((data: any) => {
                if (data) {

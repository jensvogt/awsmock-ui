...
@Component({
    selector: 'cognito-users-counter-chart-component',
    templateUrl: './cognito-users-counter-chart.component.html',
    standalone: true,
    imports: [
        ChartComponent,
        MatCard,
    ...
    styleUrls: ['./cognito-users-counter-chart.component.scss']
})
export class CognitoUsersCounterChartComponent implements OnInit {
    ...
    loadCognitoServiceTimeChart() {
        ...
        this.monitoringService.getCounters('cognito_user_counter', start, end, 5)
            .subscribe((data: any) => {
                if (data) {
                    this.cognitoServiceTimeChartOptions = {
                        series: [{name: "Users", data: data.counters}],
                        chart: {height: 350, type: "line"},
                        dataLabels: {enabled: false},
                        stroke: {show: true, curve: "smooth", width: 2},
                        tooltip: {shared: true, x: {format: "dd/MM HH:mm:ss"}},
                        title: {text: "Users", align: "center"},
